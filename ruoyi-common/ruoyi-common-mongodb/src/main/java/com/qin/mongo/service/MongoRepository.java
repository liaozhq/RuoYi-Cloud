...
public class MongoRepository {
    ...
    public <T> T findOne(Class<T> klass, Criteria criteria) {
        if (criteria == null) {
            return mongoTemplate.findOne(Query.query(criteria), klass);
        }
        return null;
    ...
    public Map findOne(String collectionName, Map<String,Object> params) {
        Criteria criteria = criteria(params);
    ...
    public Map findOne(String collectionName, Criteria criteria) {
        if (Objects.isNull(criteria)) {
            List<Map> list = mongoTemplate.findAll(Map.class, collectionName);
            if (list != null && !list.isEmpty()) {
                return list.get(0);
    ...
    public <T> Page<T> queryPage(Page<T> page, Class<T> klass) {
        return queryPage(page, klass,(Map)null);
    }

    public <T> Page<T> queryPage(Page<T> page, Class<T> klass, Map<String, Object> params) {
        Pageable pageable = org.springframework.data.domain.PageRequest.of(page.getCurrent() - 1, page.getSize(), Sort.Direction.DESC, "id");
        Query query = new Query();
    ...
    public <T> Page<T> queryPage(Page<T> page, Class<T> klass, Criteria criteria) {
        ...
        List<T> list = mongoTemplate.find(query.with(pageable), klass);
        Long count = count(klass, criteria);
        page.setTotal(count.intValue());
        page.setRecords(list);
        return page;
    }
    ...
    public <T> List<T> findAll(Class<T> klass, Criteria criteria) {
        return mongoTemplate.find(Query.query(criteria), klass);
    }
    ...
    public List<Map> findAll(String collectionName, Map<String, Object> keyValues) {
        Criteria criteria = criteria(keyValues);
        return mongoTemplate.find(Query.query(criteria), Map.class, collectionName);
    }

    public List<Map> findAll(String collectionName, Criteria criteria) {
        return mongoTemplate.find(Query.query(criteria), Map.class, collectionName);
    }
    ...
    public long count(Class klass, Criteria criteria) {
        if (criteria == null) {
    ...
    public GeoResults<Map> near(double x, double y, String collectionName, Criteria criteria) {
        return near(x,y,collectionName,criteria,10);
    }

    public GeoResults<Map> near(double x, double y, String collectionName, Criteria criteria, Integer miles) {
        Point location = new Point(x, y);
        NearQuery nearQuery = NearQuery.near(location).maxDistance(new org.springframework.data.geo.Distance(miles, Metrics.MILES));
        if (criteria != null) {
